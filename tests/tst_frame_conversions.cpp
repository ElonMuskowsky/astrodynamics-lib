...
TEST(FrameConversions, tod_to_itrf_reference_leo)
{
    ...
    CartState tod{{2057.7356919862386349, -5295.1922293517518483, -3875.5632599042232869}, {6.6963537055232232, -0.0332232476899312, 3.6167874953369825}};
    UtcJd epoch{2460676.5};

    CartState ecef = tod_to_itrf(tod, epoch);

    EXPECT_NEAR(ecef.pos.x, -5588.7617816788106211, POS_TOL);
    EXPECT_NEAR(ecef.pos.y, -1019.3520841728492314, POS_TOL);
    EXPECT_NEAR(ecef.pos.z, -3875.5632599042232869, POS_TOL);
    EXPECT_NEAR(ecef.vel.x, -1.3731622960033059, VEL_TOL);
    EXPECT_NEAR(ecef.vel.y, -6.1617302270707270, VEL_TOL);
    EXPECT_NEAR(ecef.vel.z, 3.6167874953369825, VEL_TOL);
}
